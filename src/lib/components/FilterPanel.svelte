<script lang="ts">
  ...
  function facetOptions(
    title: string,
    dimension:
      "vendors" | "materials" | "families" | "variants" | "compatiblePrinters",
    values: FilterFacet[],
  ) {
...
<aside class="filter-panel" aria-labelledby="filters-heading">
  <header class="filter-heading">
    <div>
      ...
      <h2 id="filters-heading">Source filters</h2>
    </div>
    <button
    ...
    {#each sourceApps as app (app.value)}
      <label
        ><input
          ...
          checked={filters.sourceApps.has(app.value)}
          onchange={() =>
            onFiltersChanged({
              sourceApps: toggleSetValue(filters.sourceApps, app.value),
            })}
        /><span>{app.label}</span></label
      >
    {/each}
  </fieldset>
    ...
    {#each sourceKinds as kind (kind.value)}
      <label
        ><input
          ...
          checked={filters.sourceKinds.has(kind.value)}
          onchange={() =>
            onFiltersChanged({
              sourceKinds: toggleSetValue(filters.sourceKinds, kind.value),
            })}
        /><span>{kind.label}</span></label
      >
    {/each}
  </fieldset>
  ...
  {#each groupedFacets as group (group.dimension)}
    <details
      class="facet-group"
      open={group.dimension === "vendors" || group.dimension === "materials"}
    >
      <summary
        >{group.title}<span
          >{filters[group.dimension].size || group.values.length}</span
        ></summary
      >
        ...
        {#each group.values as facet (facet.value)}
          <label>
            <input
              ...
              onchange={() =>
                onFiltersChanged({
                  [group.dimension]: toggleSetValue(
                    filters[group.dimension],
                    facet.value,
                  ),
                })}
            />
  ...
  <fieldset class="filter-group status-filter">
    ...
    {#each statuses as status (status.value)}
      <label
        ><input
          type="checkbox"
          checked={filters.migrationStatuses.has(status.value)}
          onchange={() =>
            onFiltersChanged({
              migrationStatuses: toggleSetValue(
                filters.migrationStatuses,
                status.value,
              ),
            })}
        /><span>{status.label}</span></label
      >
    {/each}
  </fieldset>
